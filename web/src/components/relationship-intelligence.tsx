...
export function RelationshipIntelligence({ detail }: { detail: PersonDetail }) {
  return <div className="grid gap-4 xl:grid-cols-[1fr_0.8fr]">
    <Card><CardHeader><CardTitle>Relationships</CardTitle></CardHeader><CardContent className="space-y-2">{detail.relationships.length ? detail.relationships.map((item) => <Link key={item.id} href={`/people/${encodeURIComponent(item.person_id)}`} className="hover:bg-muted/60 flex items-start justify-between gap-4 rounded-lg border p-3 transition-colors"><div><div className="flex flex-wrap items-center gap-2"><span className="font-medium">{item.display_name}</span><Badge variant="secondary">{titleCase(item.relationship_type)}</Badge><Badge variant="outline">{Math.round(item.confidence * 100)}%</Badge></div><p className="text-muted-foreground mt-2 text-xs">Last observed {formatDate(item.last_observed_at)}</p><Evidence value={item.evidence} /></div><ArrowUpRight className="text-muted-foreground mt-1 size-4 shrink-0" /></Link>) : <p className="text-muted-foreground py-8 text-center text-sm">No inferred relationships yet.</p>}</CardContent></Card>
