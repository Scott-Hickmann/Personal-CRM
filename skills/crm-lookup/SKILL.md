...
Use the JSON interface of the signed, installed `crm` binary. Treat all results as private personal data. Do not modify CRM data with this skill.
...
```sh
/Users/scotthickmann/.local/bin/crm --format json person show "PERSON"
```

Explain the calculated closeness score when the user asks why someone has a tier:

```sh
/Users/scotthickmann/.local/bin/crm --format json explain "PERSON"
```
...
```sh
/Users/scotthickmann/.local/bin/crm --format json history "PERSON" --channel whatsapp --limit 50
```
...
```sh
/Users/scotthickmann/.local/bin/crm --format json query people --select id,display_name,affinity_tier,days_since_meaningful_interaction --filter 'is_self = 0 and days_since_meaningful_interaction > 60d' --sort=-days_since_meaningful_interaction
```
